...
struct RegisterView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 15) {
                CustomTextField(label: "Имя", title: "Вася".localized, text: $registerData.name, isSecure: false, keyboardType: .default).disableAutocorrection(true).padding(.horizontal, 20).padding(.top, 20)
                CustomTextField(label: "Фамилия", title: "Пупкин".localized, text: $registerData.secondName, isSecure: false, keyboardType: .default).disableAutocorrection(true).padding(.horizontal, 20)
